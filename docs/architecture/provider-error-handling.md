...
> 文档状态：Current
> 权威范围：Provider 错误解析、分类、解决策略和 Session 释放行为
> 维护触发：Provider、错误模型、Parser Registry 或恢复策略变化

## 本文负责

- Provider 异常的解析、分类、策略选择和 Execution/Session 处置。
- 前台 Turn 与后台模型调用失败来源的内部表示。

## 本文不负责

- 不维护用户可见错误文案和错误码；见错误参考。
- 不定义具体 Provider SDK 的完整异常类型。
