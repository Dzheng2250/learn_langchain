...
- 每个 hunk 以 `@@` 开始；后面可带类名、函数名等语义搜索锚点，也可为空。锚点允许重复，最终仍由完整 hunk 上下文唯一定位。
...
若目标文本重复，应增加上下文或填写更具体的 `@@` 锚点，不能依赖“替换第一个匹配”。锚点只确定向后搜索的起点；即使锚点出现多次，只要完整精确上下文唯一，hunk 仍可应用。联合匹配仍有多个候选时必须拒绝。错误只返回路径、hunk 序号和原因，不返回文件正文。
