...
- 在 `create_workspace_toolset()` 中注册新工具，让 `CheckpointedToolNode` 自动接管调度、观测、预算与恢复。
...
3. **观测与恢复**：[`src/core/tools/observed.py`](/src/core/tools/observed.py) 的 `CheckpointedToolNode` 基于 `ToolNode`，按 checkpoint-safe wave 调度；`wrap_tool_call` 统一完成 ledger claim、预算扣费、telemetry 和结果持久化。工具体内部不应再做观测埋点。
...
flowchart TD
    ...
    F --> G[CheckpointedToolNode / ToolExecutionPipeline]
...
工具体**禁止**做以下事项，全部由 [`CheckpointedToolNode`](/src/core/tools/observed.py) 与
...
| 重复扣预算 | 工具体里 `budget.charge(...)` | 双扣预算 | 让 `CheckpointedToolNode` 准入、`ToolExecutionPipeline` 统一扣费 |
...
- 完整 `ToolSpec` 映射由 `create_workspace_toolset` 从 `registry.specs_for(audience)` 派生后传给 `CheckpointedToolNode` 与 `create_delegate_tool`，不能手工维护平行映射。
...
- [Agent 执行架构](/docs/architecture/agent-execution-architecture.md) — `CheckpointedToolNode` 在图中的位置。
