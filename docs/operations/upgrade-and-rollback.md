...
不要自行猜测 SQL 删除新列或降低 Schema version。Schema v11 的资源活动表属于可丢弃派生数据，确需从 v11 回到 v10 时，应使用 `learn-agent-core rollback-local-state --from-version 11 --to-version 10 --apply`；命令会拒绝运行中的 daemon，并先创建完整数据库备份。其他版本仍应恢复完整状态目录快照。
