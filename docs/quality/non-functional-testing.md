...
测试代码的目录分类、日常运行命令和新增测试归属规则见
[`/docs/quality/testing-guide.md`](/docs/quality/testing-guide.md)。
...
### 2.1 可选 PostgreSQL 集成测试

PostgreSQL 现在是可选的 Telemetry Sink、迁移来源和未来投影目标，不参与普通对话的必要提交。
因此默认测试套件不得因为 PostgreSQL 未启动而等待连接或失败。

默认测试仍会验证：

- PostgreSQL 连接参数和 URL 优先级；
- SQL 解析、参数绑定和迁移事务；
- 使用 Fake connection 的 Memory、Telemetry 和迁移行为。

只有真实读写 PostgreSQL 的集成测试需要显式启用：
...
显式集成测试要求 PostgreSQL 已启动且配置正确，并使用短连接等待上限快速暴露部署错误。
这个开关只控制测试，不是生产运行配置。

Python `unittest` 本身不提供通用的单测试超时。CI 或命令执行器仍应设置整组测试的外层超时；
任何涉及网络、数据库、线程等待或子进程的测试，还必须在测试内部使用明确的短超时，避免只能
依赖整组测试超时才能发现阻塞。
...
- Session B 的 token 输出延迟增量符合
  [`different_session_interference_ms`](/docs/quality/non-functional-requirements.md#2-延迟目标)。
